...
class ImitationBuilder(ABC):
    ...
    def build(self):
        ...
        env = FlexibleArmEnv(
            options=self.imitator_options.environment_options, estimator=self.estimator
        )

        # create safety filter
        fa_sym_sfty = SymbolicFlexibleArm3DOF(self.safety_options.n_seg)
        # fa_sym = SymbolicFlexibleArm3DOF(self.safety_options.n_seg)
        safety_filter = SafetyFilter3Dof(
            model=fa_sym_sfty, model_nonsymbolic=None, options=self.safety_options
        )
...
class ImitationBuilder_Stabilization(ImitationBuilder):
    ...
    def pre_build(self):
        ...
        self.imitator_options.environment_options.qa_range_start = np.array(
            [np.pi, np.pi, np.pi]
        )
        self.imitator_options.environment_options.qa_range_end = np.array(
            [0.0, 0.0, 0.0]
        )
        self.imitator_options.environment_options.n_seg = 3
        self.imitator_options.environment_options.render_mode = None
        self.imitator_options.environment_options.maximum_torques = np.array(
            [20, 10, 10]
        )
        ...
        self.imitator_options.n_episodes = (
            60 * 1000
        )  # number of training episodes (1000 ~ 1 minute on laptop)
        self.imitator_options.rollout_round_min_episodes = (
            5  # option for dagger algorithm.
        )
        self.imitator_options.rollout_round_min_timesteps = (
            2000  # option for dagger algorithm.
        )
        ...
        self.mpc_options.wall_constraint_on = (
        ...
        self.safety_options.nlp_iter = (
            100  # number of iterations of the nonlinear solver
        )
        self.safety_options.z_diag = np.array([0] * 3) * 1e1
        self.safety_options.z_e_diag = np.array([0] * 3) * 1e3
        self.safety_options.r_diag = np.array([1.0, 1.0, 1.0]) * 1e1
        self.safety_options.w2_slack_speed = 1e3
        self.safety_options.w2_slack_wall = 1e5
        self.safety_options.wall_constraint_on = (
            self.mpc_options.wall_constraint_on
        )  # choose whether we activate the wall constraint
        self.safety_options.wall_axis = (
            self.mpc_options.wall_axis
        )  # Wall axis: 0,1,2 -> x,y,z
        self.safety_options.wall_value = (
            self.mpc_options.wall_value
        )  # wall height value on axis
        self.safety_options.wall_pos_side = (
            self.mpc_options.wall_pos_side
        )  # defines the allowed side of the wall

        est_model = SymbolicFlexibleArm3DOF(
            n_seg=self.mpc_options.n_seg, dt=self.dt, integrator="collocation"
        )
...
class ImitationBuilder_Wall(ImitationBuilder):
    ...
    def pre_build(self):
        ...
        self.imitator_options.environment_options.qa_start = np.array(
            [-np.pi / 2 - delta_wall_angle, np.pi / 7, -np.pi / 5]
        )
        self.imitator_options.environment_options.qa_end = np.array(
            [-delta_wall_angle, np.pi / 7, -np.pi / 5]
        )
        self.imitator_options.environment_options.qa_range_start = np.array(
        ...
        self.imitator_options.rollout_round_min_episodes = (
            10  # option for dagger algorithm.
        )
        self.imitator_options.rollout_round_min_timesteps = (
            5000  # option for dagger algorithm.
        )
        ...
        self.safety_options.r_diag = np.array([1.0, 1.0, 1.0]) * 1e2
...
class ImitationBuilder_Wall2(ImitationBuilder):
    ...
    def pre_build(self):
        delta_wall_angle = np.pi / 40
        self.imitator_options.environment_options.qa_start = np.array(
            [-np.pi / 4 - delta_wall_angle, np.pi / 7, -np.pi / 5]
        )
        self.imitator_options.environment_options.qa_end = np.array(
            [-delta_wall_angle, np.pi / 7, -np.pi / 5]
        )
        ...
        self.safety_options.z_diag = np.array([0] * 3)
        self.safety_options.z_e_diag = np.array([0] * 3)
        self.safety_options.r_diag = np.array([1.0, 1.0, 1.0]) * 1
        self.safety_options.r_diag_rollout = np.array([1.0, 1.0, 1.0]) * 1e-5
        self.safety_options.w_reg_dq = 0.01
        self.safety_options.w_reg_dq_terminal = 1e-2
