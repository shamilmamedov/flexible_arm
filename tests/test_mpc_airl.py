...
now = datetime.now()
LOG_DIR = f"logs/IRL/AIRL/{now.strftime('%Y-%m-%d_%H-%M')}/SEED_{SEED}"
MODEL_DIR = f"trained_models/IRL/AIRL/{now.strftime('%Y-%m-%d_%H-%M')}/SEED_{SEED}"
...
if TRAIN_MODEL:
    ...
    learner.save(f"{MODEL_DIR}/policy_mpc_airl_last")
else:
    learner = SAC.load(f"{MODEL_DIR}/policy_mpc_airl_last")
